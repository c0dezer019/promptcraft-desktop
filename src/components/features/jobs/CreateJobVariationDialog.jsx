...
/**
 * CreateJobVariationDialog - Modal for creating a variation of a job (generation)
 * Creates a new generation job marked as variation, NOT a scene
 */
export function CreateJobVariationDialog({ parentJob, onClose, onCreateVariation }) {
  const jobData = parentJob.data;
  const [mainPrompt, setMainPrompt] = useState(jobData?.prompt || '');
  const [negativePrompt, setNegativePrompt] = useState(jobData?.negative_prompt || '');
  const [notes, setNotes] = useState('');
  const [useAsReference, setUseAsReference] = useState(true);
  const [creating, setCreating] = useState(false);

  // Determine if this model supports negative prompts
  const modelId = jobData?.model || '';
  ...
  const handleCreate = async () => {
    setCreating(true);
    try {
      await onCreateVariation(parentJob, {
        prompt: {
          main: mainPrompt,
          negative: supportsNegativePrompt ? negativePrompt : undefined,
        },
        useAsReference,
  ...
  // Get image preview from job result
  const jobResult = parentJob.result;
  const imagePreview = jobResult?.output_url || (jobResult?.output_data ? `data:image/png;base64,${jobResult.output_data}` : null);
        ...
        <div className="p-6 space-y-6 max-h-[70vh] overflow-y-auto">
          ...
          <div className="bg-purple-50 dark:bg-purple-900/20 border border-purple-200 dark:border-purple-800 rounded-lg p-4">
            <p className="text-sm text-purple-900 dark:text-purple-200">
              <strong>Note:</strong> This will create a new generation in your history.
              You can save it as a scene later if you want to evolve it further.
            </p>
          </div>
        </div>
